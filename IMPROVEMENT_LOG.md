...
Frontend-only enhancement queue for the live app. Implementers work entries **top to bottom**;
locally approved entries batch uncommitted and ship together as one squashed commit. Update each
entry's `Status:`/`Notes:` as rule 10 requires. This file holds **live work only** — shipped entries
are archived verbatim under `docs/archive/`, indexed by `IMPROVEMENTS.md`, the audit trail and never
a queue.
...
10. **Bookkeeping:** set `Status: In progress — date` when starting; on local approval set `Status: Done — date`, record a summary plus any deviations in `Notes:`, and update the queue index. If an entry cannot be completed inside these rules, set `Status: Blocked — reason` and move on — do not bend the rules. **Batching:** `Done` means implemented and reviewed *locally*, not committed. Entries accumulate as an open batch and ship as **one squashed commit** when `npm run queue` reports exit `6`: at `BATCH_MIN` waiting entries, or when an operator forces a non-empty tail with `--ship`. An unshipped entry has no ``Commit `<subject>`.`` note; the ship step adds that exact subject to every entry in the batch. This file carries at most `BATCH_MAX` unshipped `Done` entries, enforced against the constant in `scripts/queue-status.mjs`. **Archiving:** before starting new work, move any `archive-due:` entry (heading through its `---` separator, **verbatim**) into the current archive file, the one `IMPROVEMENTS.md` marks `current`, and drop its index line here. A ship step archives every entry in its batch in the same commit; until then an unshipped entry has nothing to archive. `IMPROVEMENTS.md` is only the index over `docs/archive/`; never paste an entry into it. If the current archive is at the cap `tests/docs-check.mjs` enforces, start the next pass file and mark it current instead of raising the cap.
