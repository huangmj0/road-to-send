...
Notes: Commit `Re-baseline the bundle budget`. `BUDGET` moved 156,000 → 161,000 against the
measured 135,867-byte artifact after entry 22, the ~12,600-byte projection for entries 25–41 and
the resulting ~12,500-byte cushion at a landing near 148,500; the standing instruction to lower
the next budget toward the real figure if the queue again lands well under projection remains.
`index.html` stayed byte-identical at 135,867 bytes (84.4% of the new budget; hash
`ef8db6b4375ac20d641bc3bac155fe78e1fe1a43`) after `npm run build`; `npm test` passed 5/5 suites.
Deviations: none. Rule 10 archiving: entry 22 was moved verbatim into `IMPROVEMENTS.md` after
archived entry 21 and its queue-index line was dropped.
