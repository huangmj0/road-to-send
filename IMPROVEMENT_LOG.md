...
- 58 — Caption the claimed bounty list — Done — 2026-08-01
...
Notes: Commit `Caption the claimed bounty list`. Added `claimedCaption(rows)` in `src/app.js`,
following `heatmapCaption()`/`trendCaption()`/`pyramidCaption()`: pure, no DOM. Added
`<p id="claimedSummary" class="hint"></p>` right after `#claimedList` in
`src/index.template.html`. `renderClaimed()` now sets `#claimedSummary`'s `textContent` to
`claimedCaption(rows)` when open and clears it on the closed branch, guarded the way the rest of
that function guards its elements. index.html 152,071 → 152,431 bytes (+360, 92.4% of the
