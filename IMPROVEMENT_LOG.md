...
- 85 — Give the Bounty Hunter marker visible text and the small controls a 44px target — Done — 2026-08-01
- 86 — Count a bounty-only day as a day you were active — Todo
...
### Requirements
- `src/app.js` only, plus tests. The fix is entirely client-side.
- Use the existing `nameKey(x)` helper for the personal-feed filter at `src/app.js:114` instead of `x.name.toLowerCase()`, so it matches how the totals were computed.
- In `sanitizeActivities()` (`src/app.js:19`), coerce each row's `name` to a trimmed string as it enters `logs`, so every later reader sees the same shape. Keep the existing `type` validation exactly as it is, and keep rows whose name is empty after trimming behaving as they do today — this entry hardens the type, it does not add a new drop rule.
...
### Tests
- `tests/client-state.state.test.js`: `sanitizeActivities()` returns a trimmed string name for a row whose name has surrounding whitespace and for a row whose name is a number.
...
### Do not
Do not touch `src/apps-script.js`, `src/schema.json` or `src/scoring.json` (rule 2). Do not rename or re-key anything in localStorage (rule 4). Do not silently discard rows with unexpected names — the crew's data stays visible; this is about reading it consistently. Do not add a new user-facing warning about malformed rows; the diagnostics surface already exists and this entry does not add copy to it. Tone rule: removing a false empty state is the whole point — do not replace it with any message about what someone has not logged.
...
- An `<option>` carries no styling hook, so the marker is plain text inside the option label. Keep it identical in wording to the You tab's marker so the two surfaces read the same.
...
- Add title bullets for entries 41–60, 61–82 and 83–84 under their existing sections in `IMPROVEMENTS.md`, matching the format the 1–40 sections already use. Take the titles verbatim from each pass file's `## N.` headings.
