# Pre-queue priorities (v6–v11)

The P0–P3 backlog that predates the four-state entry protocol, kept for the reasoning behind the
challenge window, the coverage work and the bounty catalog. Statuses here are free text ("resolved
in API v9"), not the `Todo`/`In progress`/`Done`/`Blocked` vocabulary the queue uses now.
