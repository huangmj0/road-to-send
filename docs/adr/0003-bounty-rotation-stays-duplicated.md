...
Once the build tool in ADR-0001 exists, injecting one shared module into both will be possible. We
still would not do it. Each crew's organizer deploys the Apps Script by hand, so a shared module
only protects a backend once that organizer redeploys — the fix is slow-acting and arrives crew by
crew, while the drift it guards against is immediate.
